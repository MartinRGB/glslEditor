...
void main() {
    ...
    vec2 q = vec2(0.);
    float texScale = 5.5;
    q.x = texture2D(u_tex6, (st + 0.00*u_time ) / texScale).x;
    q.y = texture2D(u_tex6, (st + vec2(1.0)) / texScale).y;

    vec2 r = vec2(0.);
    r.x = texture2D(u_tex6, (st + 1.0*q + vec2(0.330,-0.660)+ 0.15*sin(u_time)) / texScale).x;
    r.y = texture2D(u_tex6, (st + 1.0*q + vec2(0.140,-0.430)+ 0.126*cos(u_time)) / texScale).y;

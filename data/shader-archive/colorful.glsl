...
//    vec2 st =  vec2(texCoord.x/234.*108./234.*108.,(1.0-texCoord.y/234.*108.))*4.;
    ...
    vec2 q = vec2(0.);
    float texScale = 5.5;
    q.x = texture2D(u_tex5, (st +  0.00 ) / texScale).x;
    q.y = texture2D(u_tex5, (st + vec2(1.0)) / texScale).y;

    vec2 r = vec2(0.);
    r.x = texture2D(u_tex5, (st + 1.0*q + vec2(0.330,-0.660) + 0.15*(u_time)*1.0) / texScale).x;
    r.y = texture2D(u_tex5, (st + 1.0*q + vec2(0.140,-0.430) + 0.126*(u_time)*1.0) / texScale).y;

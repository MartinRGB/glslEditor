...
#define uColorValue3 vec4(0.0,1.0,0.0,1.0)
...
vec3 foggyComputer(in vec2 st){
    ...
    q.y = texture2D(iChannel0, (st + vec2(1.0)*u_slot1) / texScale).y;

    vec2 r = vec2(0.);
    r.x = texture2D(iChannel0, (st + 1.0*q*u_slot1 + vec2(0.330,-0.660)*u_slot1 + 0.15*(u_time)*1.0 ) / texScale).x;
    r.y = texture2D(iChannel0, (st + 1.0*q*u_slot1 + vec2(0.140,-0.430)*u_slot1 + 0.126*(u_time)*1.0  ) / texScale).y;
    ...
    float h = texture2D(iChannel0, (st + r ) / texScale).z;
     ...
     color = mix(vec3(st.x, uColorValue3.x, uColorValue3.y)*u_slot1,
                   vec3(st.y, uColorValue3.z, uColorValue3.w)*u_slot1,
                   clamp((f * f) * 1.024, 0.0, 1.0));
     color = mix(color*u_slot1, uColorValue1*u_slot1, clamp(length(q), 0., 0.616));
     color = mix(color*u_slot1, uColorValue2*u_slot1, clamp(length(r.x), 0., 0.624));
     vec3 finalColor = (f * f * f + .6 * g + .5 * h) * color*u_slot1;
     finalColor.r = clamp(finalColor.r, 0.0, 0.9);
...
void main() {
     ...
     st+= -vec2(0.5,0.0)*u_slot1;
    
//     float k = 1.0 * sin( u_time * .9 );
//     float kcube = .5 * sin( u_time );
    
//     float offset = .1 * sin( u_time * .5 );
     ...
     finColor.x = foggyComputer(computeUV( st, k + offset, kcube )).r;
     finColor.y = foggyComputer(computeUV( st, k, kcube )).g;
     finColor.z = foggyComputer(computeUV( st, k - offset, kcube )).g;

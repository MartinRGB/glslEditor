...
#define uSpaceTranslate vec2(0.290,0.490)


#define uColorValue1 vec3(0.44,0.0,0.46)
#define uColorValue2 vec3(0.0,0.32,0.74)
#define uColorValue3 vec4(.0,0.0,1.0,0.0)
#define uBrightness 0.
...
void main() {
    ...
    vec2 r = vec2(0.);
    r.x = texture2D(uTexture, (st + 1.0*q*u_slot1*u_slot1 + vec2(0.400,-0.420)*u_slot1*u_slot1 + 0.15 * (u_time)*uSpeed + vec2(pageturnDisplacement/2.,0.)) / texScale).x;
    r.y = texture2D(uTexture, (st + 1.0*q*u_slot1*u_slot1 + vec2(-0.140,0.910)*u_slot1*u_slot1 + 0.126* (u_time)*uSpeed + vec2(pageturnDisplacement/2.,0.)) / texScale).y;

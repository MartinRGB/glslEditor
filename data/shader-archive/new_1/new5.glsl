...
#define uSpaceScale 9.928
#define uSpaceTranslate vec2(0.010,0.650)*10.


#define uColorValue1 vec3(0.44,0.0,0.46)
#define uColorValue2 vec3(0.0,0.32,0.74)
#define uColorValue3 vec4(1.0,1.0,1.0,1.0)
...
void main() {
    ...
    vec2 q = vec2(0.);
    float texScale = 5.5;
    q.x = texture2D(uTexture, (st  ) / texScale).x;
    q.y = texture2D(uTexture, (st + vec2(-0.100,-1.000)  ) / texScale).y;
    q+= vec2(u_slot1*u_slot1*0.5 - 0.5  + pageturnDisplacement/5.  ,u_slot1*u_slot1*0.5 - 0.5);

    vec2 r = vec2(0.);
    r.x = texture2D(uTexture, (st + 1.0*q*u_slot1*u_slot1 + vec2(-0.980,-0.950)*u_slot1*u_slot1 + 0.15 * (u_time)*uSpeed + vec2(pageturnDisplacement/2.,0.)) / texScale).x;
    r.y = texture2D(uTexture, (st + 1.0*q*u_slot1*u_slot1 + vec2(-0.610,-0.520)*u_slot1*u_slot1 - 0.15* (u_time)*uSpeed + vec2(pageturnDisplacement/2.,0.)) / texScale).y;
    ...
    color = mix(color, uColorValue1, clamp(length(q), 0.5, 0.616));
    color = mix(color, uColorValue2, clamp(length(r.y), 0.5, 0.624));
    vec3 finalColor = (f * f * f + .6 * g + .5 * h) * color;
    finalColor.r = clamp(finalColor.r, 0.2, 1.0);
    //finalColor.r = clamp(finalColor.r, 0.0, 0.7);
    //finalColor.g = clamp(finalColor.g, 0.0, 0.9);
